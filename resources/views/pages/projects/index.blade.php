                ...
                <div class="flex flex-wrap items-center gap-x-5 gap-y-3">
                    <div class="flex flex-wrap gap-1.5" role="group" aria-label="فیلتر نوع پروژه">
                        <button type="button" @click="setFilter('all')"
                                ...
                                class="rounded-full px-3.5 py-1.5 text-[0.875rem] font-semibold transition">
                            همه
                            <span class="tech mr-1 text-[0.75rem] opacity-60">{{ \App\Support\Jalali::digits($projects->count()) }}</span>
                        </button>
                        @foreach($categories as $category)
                            <button type="button" @click="setFilter('{{ $category->slug }}')"
                                    ...
                                    class="rounded-full px-3.5 py-1.5 text-[0.875rem] font-semibold transition">
                                {{ $category->name }}
                                <span class="tech mr-1 text-[0.75rem] opacity-60">{{ \App\Support\Jalali::digits($category->projects_count) }}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
