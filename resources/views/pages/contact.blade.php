<x-layouts.app>
    @php
        // کلیدها در دیتابیس ذخیره می‌شوند، پس کدند؛ برچسبشان متن است
        $types = collect(['quote', 'technical', 'distributor', 'general'])
            ->mapWithKeys(fn (string $key) => [$key => [
                'label' => __("site.contact.type.{$key}.label"),
                'hint' => __("site.contact.type.{$key}.hint"),
            ]])
            ->all();
    @endphp
    ...
    <x-page-hero
        eyebrow="Contact"
        :title="__('site.nav.items.contact')"
            ...
            <div class="lg:col-span-7">
                ...
                @if($errors->any())
                    <div role="alert" class="mb-6 rounded-[var(--radius-panel)] border border-red-300 bg-red-50 p-5">
                        <p class="font-bold text-red-800">{{ __('site.contact.fix') }}</p>
                ...
                <form method="POST" action="{{ route('contact.store') }}"
                    ...
                    <div class="hidden" aria-hidden="true">
                        <label for="website">{{ __('site.contact.website') }}</label>
                        <input id="website" name="website" type="text" tabindex="-1" autocomplete="off">
                    </div>

                    <fieldset>
                        <legend class="text-meta font-semibold text-ink-600">{{ __('site.contact.subject') }}</legend>
                    ...
                    <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        @foreach([
                            ['name', 'text', true, 'name'],
                            ['company', 'text', false, 'organization'],
                            ['phone', 'tel', true, 'tel'],
                            ['email', 'email', false, 'email'],
                            ['city', 'text', false, 'address-level2'],
                        ] as [$field, $type, $required, $autocomplete])
                            @php $label = __("site.contact.field.{$field}"); @endphp
                            <div @class(['sm:col-span-2' => $field === 'city'])>
                                ...
                                <input id="{{ $field }}" name="{{ $field }}" type="{{ $type }}"
                                       ...
                                       @if($field === 'phone') inputmode="tel" dir="ltr" placeholder="{{ __('site.contact.phone_placeholder') }}" @endif
                        ...
                        <div class="sm:col-span-2">
                            <label for="product_id" class="mb-2 block text-meta font-semibold text-ink-600">{{ __('site.contact.field.product') }}</label>
                            <div class="relative">
                                <select id="product_id" name="product_id"
                                    ...
                                    <option value="">{{ __('site.contact.none') }}</option>
                        ...
                        <div class="sm:col-span-2">
                            <label for="message" class="mb-2 block text-meta font-semibold text-ink-600">
                                {{ __('site.contact.field.message') }} <span class="text-clay-600" aria-hidden="true">*</span>
                            </label>
                    ...
                    <div class="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-sand-200 pt-6">
                        <p class="text-meta text-ink-400">{{ __('site.contact.required_note') }}</p>
                        <button type="submit"
            ...
            <aside class="lg:col-span-5">
                <div class="sticky top-28 space-y-4">
                    <div class="rounded-[var(--radius-panel)] border border-sand-300 bg-sand-50 p-6">
                        <h2 class="font-bold">{{ __('site.contact.direct') }}</h2>
                    ...
                    <div class="rounded-[var(--radius-panel)] border border-sand-300 bg-sand-100 p-6">
                        ...
                        <ul class="mt-4 space-y-2">
                            @foreach([
                                'finder' => 'finder.show',
                                'downloads' => 'technical.downloads',
                                'faq' => 'technical.faq',
                                'distributor' => 'distributors',
                            ] as $key => $route)
                                @php $label = __("site.contact.shortcut.{$key}"); @endphp
                                <li>
                                    <a href="{{ route($route) }}"
