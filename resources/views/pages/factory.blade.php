                ...
                <div class="relative overflow-hidden rounded-[var(--radius-panel)] border border-ink-900/10 bg-ink-950 lg:col-span-8">
                    ...
                    @foreach($sections as $index => $section)
                        <button type="button" x-ref="spot{{ $index }}" @click="select({{ $index }})"
                            ...
                            <span class="pointer-events-none absolute left-9 top-1/2 hidden -translate-y-1/2 whitespace-nowrap rounded-lg bg-ink-900/90 px-2.5 py-1 text-micro text-sand-50 opacity-0 backdrop-blur transition-opacity group-hover:opacity-100 lg:block"
                ...
                @foreach($certificates as $certificate)
                    <li data-reveal class="flex gap-4 rounded-2xl border border-sand-300 bg-sand-100 p-5">
                        ...
                        <div>
                            ...
                            @if($certificate->year)
                                <p class="tech mt-1.5 text-micro text-ink-300">{{ \App\Support\Jalali::digits($certificate->year) }}</p>
                            @endif
                        </div>
                    </li>
                @endforeach
