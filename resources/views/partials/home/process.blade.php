@php
    ...
    */
    ...
    | باریک‌ترین حالتشان. از تعداد کارت‌ها می‌آید، پس اینجا حساب می‌شود و نه در
    | CSS — تا با کم و زیاد شدن مراحل خودش را اصلاح کند. هم‌تراز با ثابت‌های
    | resources/js/alpine/squeeze-carousel.js.
    */
    $fixed = ($slides->count() - 1) * 12 + max(0, $slides->count() - 4) * 8;
@endphp

{{--
    «از خاک تا سازه» — کاروسل فشرده.

    دسکتاپ: ساختار ردیف در هر ده حالت یکی است — نوارهای رد شده، کارتِ مربعِ
    باز، بادبزنِ سه‌تایی به سمت چپ، و نوارهای نرسیده. کارتِ باز و هر کارت
    بادبزن در همه‌ی حالت‌ها دقیقاً یک عرض دارند؛ تنها چیزی که عوض می‌شود پهنای
    نوارهاست، که روی مراحل آخر — که دیگر کارتی جلوتر نمانده — باز می‌شوند و
    جای خالی را می‌گیرند. هیچ کارتی پنهان نمی‌شود و با کلیک هرکدام، همان باز
    می‌شود.
    ...
    <div class="sq-carousel container-page relative"
        ...
        <div class="mt-7 lg:overflow-hidden">
            <ol x-ref="strip"
                ...
                style="--sq-fixed: {{ $fixed }}px; --sq-slat: 8px"
