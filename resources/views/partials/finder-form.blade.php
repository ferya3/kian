...
<div x-data="productFinder('{{ route('finder.results') }}')" class="w-full">
    <form x-ref="form" action="{{ route('finder.results') }}" method="GET"
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-4">
            @php
                ...
                $fields = [
                    ['name' => 'project_type', 'options' => Options::finder('project_types')],
                    ['name' => 'wall_type', 'options' => Options::finder('wall_types')],
                ];
            @endphp
            ...
            @foreach($fields as $field)
                <div>
                    <label for="finder-{{ $field['name'] }}" class="mb-2 block text-meta font-semibold text-ink-600">
                        {{ __('site.finder.field.'.$field['name']) }}
                    </label>
                    <div class="relative">
                        <select id="finder-{{ $field['name'] }}" name="{{ $field['name'] }}"
                            ...
                            <option value="">{{ __('site.finder.choose') }}</option>
            ...
            <div>
                <label for="finder-thickness" class="mb-2 block text-meta font-semibold text-ink-600">
                    {{ __('site.finder.field.thickness') }}
                </label>
                <div class="relative">
            ...
            <div>
                <label for="finder-insulation" class="mb-2 block text-meta font-semibold text-ink-600">
                    {{ __('site.finder.field.insulation') }}
                </label>
                <div class="relative">
                    <select id="finder-insulation" name="insulation" x-model="criteria.insulation"
                            class="w-full appearance-none rounded-xl border border-sand-300 bg-sand-100 py-3.5 pr-4 pl-10 font-semibold text-ink-800 outline-none transition focus:border-clay-400 focus:bg-sand-50">
                        <option value="">{{ __('site.finder.any') }}</option>
                        @foreach(Options::finder('insulation_levels') as $value => $level)
    ...
    <div x-ref="results" x-html="results" class="focus:outline-none"
         role="region" aria-live="polite" aria-label="{{ __('site.finder.results_region') }}"></div>

    <noscript>
        <p class="mt-3 text-meta text-ink-400">
            {{ __('site.finder.noscript') }}
        </p>
    </noscript>
</div>
